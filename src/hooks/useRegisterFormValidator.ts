...
import { ErrorsType, FormRegisterType } from '@utils/type'
import { confirmPasswordValidator, emailValidator, passwordValidator, usernameValidator } from '@utils/validator'
...
export const useRegisterFormValidator = (form: FormRegisterType) => {
  ...
  const validateForm = ({
    ...
    if (nextErrors.username.dirty && (field ? field === 'username' : true)) {
      const usernameMessage = usernameValidator(form.username)
    ...
    if (nextErrors.confirmPassword.dirty && (field ? field === 'confirmPassword' : true)) {
      const confirmPasswordMessage = confirmPasswordValidator(form.confirmPassword, form)
